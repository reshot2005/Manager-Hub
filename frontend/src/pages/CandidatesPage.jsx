...
const STAGE_CONFIG = {
  ...
  Screening: { color: '#6c4dff', bg: '#f3f0ff' },
    ...
    <div ref={pageRef} className="relative flex h-full flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex h-full flex-col p-6 md:p-8">
        ...
        <div className="cand-header flex items-center justify-between mb-5">
          <div>
            <h1 className="text-[26px] font-bold tracking-tight text-[#1f1f2e]">Candidates</h1>
        ...
        {/* Pipeline funnel */}
        {rows.length > 0 && (
          <div className="cand-funnel rounded-2xl border border-[#EDEDF5] bg-white p-5 mb-5 shadow-[0_2px_8px_rgba(15,23,42,0.05)]">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-semibold text-[#1f1f2e]">Pipeline Overview</div>
              <div className="flex flex-wrap gap-1.5">
                <button
                  onClick={() => setStageFilter('')}
                  className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium transition ${!stageFilter ? 'bg-[#6c4dff] text-white' : 'bg-[#f3f4f6] text-[#6b7280] hover:bg-[#f3f0ff]'}`}
                >
                  All
                </button>
                {STAGE_ORDER.map((s) => (
        ...
        <div className="relative mb-5 max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-[#EDEDF5] bg-white py-2.5 pl-10 pr-3 text-sm outline-none focus:border-[#8b5cf6] focus:ring-2 focus:ring-[#8b5cf6]/10"
          />
        </div>
        ...
        <div className="grid min-h-0 flex-1 gap-4 md:grid-cols-5">
                ...
                {filtered.map((c) => (
                  <tr
                    ...
                    className={`cursor-pointer border-t border-[#EDEDF5] transition hover:bg-[#f7f7fb] ${
                      selected === c.id ? 'bg-[#f3f0ff]' : ''
                    }`}
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#8b5cf6] to-[#6c4dff] text-xs font-bold text-white shrink-0">
                          {c.name?.split(' ').map((p) => p[0]).join('').slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                          <div className="font-semibold text-[#1f1f2e]">{c.name}</div>
          ...
          {/* Detail panel */}
          <div className="overflow-auto rounded-2xl border border-[#EDEDF5] bg-white shadow-[0_2px_12px_rgba(15,23,42,0.06)] md:col-span-2">
            {detailLoading && (
            ...
            {detail && !detailLoading && (
              <div className="flex flex-col h-full">
                {/* Profile */}
                <div className="p-5 border-b border-[#EDEDF5] bg-gradient-to-br from-[#faf5ff] to-white">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#8b5cf6] to-[#6c4dff] text-base font-bold text-white shadow-lg">
                      {detail.candidate.name?.split(' ').map((p) => p[0]).join('').slice(0, 2).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h2 className="text-lg font-bold text-[#1f1f2e]">{detail.candidate.name}</h2>
                ...
                {detail.applications?.length > 0 && (
                  <div className="px-5 py-4 border-b border-[#EDEDF5]">
                    ...
                    <div className="space-y-2">
                      {detail.applications.map((a, i) => (
                        <div key={i} className="rounded-xl border border-[#EDEDF5] bg-[#f7f7fb] px-3.5 py-3">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <div className="text-sm font-semibold text-[#1f1f2e]">{a.job_title || 'Unknown Role'}</div>
                  ...
                  {!detail.interviews?.length ? (
                    ...
                    <div className="space-y-3">
                      {detail.interviews.map((i, idx) => (
                        <div key={idx} className="rounded-xl border border-[#EDEDF5] bg-[#f7f7fb] p-3.5">
                          ...
                          {i.meeting_link && (
                            <a
                              ...
                              className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-[#6c4dff] hover:underline"
                            >
                              Join Meeting →
                            </a>
                          )}
